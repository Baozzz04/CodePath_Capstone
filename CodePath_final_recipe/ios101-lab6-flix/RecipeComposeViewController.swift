...
//    }
    ...
    var onComposeTask: ((Movie) -> Void)? = nil
    ...
    override func viewDidLoad() {
        ...
        if let task = taskToEdit {
            ...
            noteField.text = task.overview
            ingredientsField.text = task.ingred
            linkField.text = task.posterPath
...
//            // ii.
//            task.title = title
//            task.overview = noteField.text!
//            task.ingred = ingredientsField.text
//            task.posterPath = linkField.text
//        } else {
//            // 4.
//            task = Movie(title: title,
//                         overview: noteField.text!,
//                         posterPath: linkField.text,
//                         backdropPath: "path_to_backdrop_image",
//                         ingred: ingredientsField.text,
//                         voteAverage: 9.0, releaseDate: Date())
//        }
        
        // 2. Create a new movie instance with the entered details
        let newMovie = Movie(
            title: title,
            overview: noteField.text ?? "",
            posterPath: linkField.text ?? "",
            backdropPath: "path_to_backdrop_image",
            ingred: ingredientsField.text ?? "",
            voteAverage: 0.0, // Set default value, you might want to change this
            releaseDate: Date()
        )
        
        // 3. Add the new movie to the hardcoded movies array
        movies.append(newMovie)
        
        // 4. Save the updated hardcoded movies array
        Movie.save(movies, forKey: "Favorites")
        
        // 5.
        onComposeTask?(newMovie)
        // 6.
